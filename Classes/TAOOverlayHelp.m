...
@interface TAOOverlayHelp () {
  BOOL layoutSubviewsAlreadyCalled;
}
...
-(void) layoutSubviews
{
  if (!layoutSubviewsAlreadyCalled) {
    [super layoutSubviews];
    [self setNeedsLayout];
    layoutSubviewsAlreadyCalled = TRUE;
  } else {
    [self dismiss];
  }
}
...
- (void)setUpConstraintsForLabel {
  ...
  NSLayoutConstraint *width = [NSLayoutConstraint constraintWithItem:self.textLabel attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationLessThanOrEqual toItem:self attribute:NSLayoutAttributeWidth multiplier:1.0f constant:0.0f];
  
  [NSLayoutConstraint activateConstraints:@[width, centerX, centerY]];
  [self.textLabel layoutIfNeeded];
}

- (void)setUpConstraints {
  NSLayoutConstraint* marginLeft = [NSLayoutConstraint constraintWithItem:self attribute:NSLayoutAttributeLeading relatedBy:NSLayoutRelationGreaterThanOrEqual toItem:self.superview attribute:NSLayoutAttributeLeading multiplier:1 constant:0];
  NSLayoutConstraint* marginRight = [NSLayoutConstraint constraintWithItem:self attribute:NSLayoutAttributeTrailing relatedBy:NSLayoutRelationLessThanOrEqual toItem:self.superview attribute:NSLayoutAttributeTrailing multiplier:1 constant:0];
  NSLayoutConstraint* marginTop = [NSLayoutConstraint constraintWithItem:self attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationGreaterThanOrEqual toItem:self.superview attribute:NSLayoutAttributeTop multiplier:1 constant:0];
  NSLayoutConstraint* marginBottom = [NSLayoutConstraint constraintWithItem:self attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationGreaterThanOrEqual toItem:self.superview attribute:NSLayoutAttributeBottom multiplier:1 constant:0];
  
  [NSLayoutConstraint activateConstraints:@[marginLeft, marginRight, marginTop, marginBottom]];
}
...
- (void)updateConstraints {
  /* NSLayoutConstraint* centerY = [NSLayoutConstraint constraintWithItem:self.textLabel attribute:NSLayoutAttributeCenterY relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeCenterY multiplier:1 constant:0];
   ...
   NSLayoutConstraint* marginTop = [NSLayoutConstraint constraintWithItem:self.textLabel attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationGreaterThanOrEqual toItem:self attribute:NSLayoutAttributeTop multiplier:1 constant:10];
   NSLayoutConstraint* marginBottom = [NSLayoutConstraint constraintWithItem:self.textLabel attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationGreaterThanOrEqual toItem:self attribute:NSLayoutAttributeBottom multiplier:1 constant:-10];
   
   [NSLayoutConstraint activateConstraints:@[centerX, centerY, marginLeft, marginRight, marginTop, marginBottom]];
   */
  //[self setUpConstraintsForLabel];
  [self.textLabel updateConstraints];
...
+ (NSArray*)fillSuperviewConstraintsForView:(UIView*)view {
  ...
  for (NSNumber* attribute in attributes) {
    NSLayoutConstraint* constraint = [NSLayoutConstraint constraintWithItem:view attribute:[attribute integerValue] relatedBy:NSLayoutRelationEqual toItem:view.superview attribute:[attribute integerValue] multiplier:1 constant:0];
